    ...
    <div class="space-y-6">
      ...
      <template v-else>
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-4 dark:border-dark-700 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 class="text-lg font-semibold text-gray-900 dark:text-white">API Key 异常</h2>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">查看异常用户、Key、触发规则和封禁状态。</p>
            </div>
            <button type="button" class="btn btn-secondary inline-flex items-center gap-2" :disabled="apiKeyRiskLoading" @click="loadAPIKeyRiskEvents">
              <Icon name="refresh" size="sm" :class="apiKeyRiskLoading ? 'animate-spin' : ''" />
              刷新
            </button>
          </div>
          ...
          <div class="space-y-4 p-6">
            <div class="grid grid-cols-1 gap-3 md:grid-cols-4">
              <input v-model.trim="apiKeyRiskFilters.search" type="search" class="input" placeholder="搜索用户邮箱、Key 名称、规则" @keyup.enter="reloadAPIKeyRiskFromFirstPage" />
              <Select v-model="apiKeyRiskFilters.status" :options="apiKeyRiskStatusOptions" @change="reloadAPIKeyRiskFromFirstPage" />
              <Select v-model="apiKeyRiskFilters.severity" :options="apiKeyRiskSeverityOptions" @change="reloadAPIKeyRiskFromFirstPage" />
              <Select v-model="apiKeyRiskFilters.rule_code" :options="apiKeyRiskRuleOptions" @change="reloadAPIKeyRiskFromFirstPage" />
            </div>
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
                ...
                <tbody class="divide-y divide-gray-100 bg-white dark:divide-dark-700 dark:bg-dark-800">
                  <tr v-if="apiKeyRiskEvents.length === 0">
                    <td colspan="6" class="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">暂无 API Key 异常事件</td>
                  </tr>
                  <tr v-for="event in apiKeyRiskEvents" :key="event.id">
                    ...
                    <td class="max-w-md px-5 py-4 text-xs text-gray-600 dark:text-gray-300">
                      <p class="truncate">{{ apiKeyRiskEvidenceSummary(event) }}</p>
                      <p class="mt-1 truncate font-mono text-gray-400">{{ apiKeyRiskEvidenceSamples(event) }}</p>
                    </td>
                    <td class="px-5 py-4">
                      <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium" :class="apiKeyRiskStatusClass(event.status)">
                        {{ event.status }}
                      </span>
                    </td>
                    <td class="px-5 py-4 text-right">
                      <div class="flex justify-end gap-2">
                        <button type="button" class="btn btn-secondary btn-sm" :disabled="apiKeyRiskActionID === event.id" @click="resolveAPIKeyRiskEvent(event)">处理</button>
                        <button v-if="event.status === 'blocked'" type="button" class="btn btn-primary btn-sm" :disabled="apiKeyRiskActionID === event.id" @click="unblockAPIKeyRiskEvent(event)">解封 Key</button>
                      </div>
                    </td>
                  </tr>
                </tbody>
            ...
            <Pagination
              v-if="apiKeyRiskPagination.total > 0"
              :page="apiKeyRiskPagination.page"
              :total="apiKeyRiskPagination.total"
              :page-size="apiKeyRiskPagination.page_size"
              @update:page="onAPIKeyRiskPageChange"
              @update:page-size="onAPIKeyRiskPageSizeChange"
            />
          </div>
        </div>
...
const apiKeyRiskEvents = ref<APIKeyRiskEvent[]>([])
...
const apiKeyRiskPagination = reactive({
  page: 1,
  page_size: 20,
  total: 0,
  pages: 1,
...
const apiKeyRiskFilters = reactive({
  search: '',
  status: '',
  severity: '',
  rule_code: '',
...
const apiKeyRiskStatusOptions = computed<SelectOption[]>(() => [
  { value: '', label: '全部状态' },
...
const apiKeyRiskSeverityOptions = computed<SelectOption[]>(() => [
  { value: '', label: '全部级别' },
  { value: 'high', label: '高危' },
  { value: 'medium', label: '中危' },
])

const apiKeyRiskRuleOptions = computed<SelectOption[]>(() => [
  { value: '', label: '全部规则' },
...
async function loadAll() {
  ...
  try {
    ...
    await Promise.all([loadLogs(), loadAPIKeyRiskEvents()])
  } catch (err: unknown) {
...
async function loadAPIKeyRiskEvents() {
  apiKeyRiskLoading.value = true
  try {
    const result = await adminAPI.riskControl.listAPIKeyRiskEvents({
      page: apiKeyRiskPagination.page,
      page_size: apiKeyRiskPagination.page_size,
      search: apiKeyRiskFilters.search || undefined,
      status: apiKeyRiskFilters.status || undefined,
      severity: apiKeyRiskFilters.severity || undefined,
      rule_code: apiKeyRiskFilters.rule_code || undefined,
    })
    apiKeyRiskEvents.value = result.items
    apiKeyRiskPagination.total = result.total
    apiKeyRiskPagination.page = result.page
    apiKeyRiskPagination.page_size = result.page_size
    apiKeyRiskPagination.pages = result.pages
  } catch (err: unknown) {
    appStore.showError(extractApiErrorMessage(err, '加载 API Key 异常事件失败'))
  } finally {
    apiKeyRiskLoading.value = false
  }
}

function reloadAPIKeyRiskFromFirstPage() {
  apiKeyRiskPagination.page = 1
  void loadAPIKeyRiskEvents()
}

function onAPIKeyRiskPageChange(page: number) {
  apiKeyRiskPagination.page = page
  void loadAPIKeyRiskEvents()
}

function onAPIKeyRiskPageSizeChange(pageSize: number) {
  apiKeyRiskPagination.page = 1
  apiKeyRiskPagination.page_size = pageSize
  void loadAPIKeyRiskEvents()
}

async function resolveAPIKeyRiskEvent(event: APIKeyRiskEvent) {
  if (apiKeyRiskActionID.value !== null) return
  apiKeyRiskActionID.value = event.id
  try {
    await adminAPI.riskControl.resolveAPIKeyRiskEvent(event.id)
    await loadAPIKeyRiskEvents()
    appStore.showSuccess('已标记处理')
  } catch (err: unknown) {
    appStore.showError(extractApiErrorMessage(err, '处理 API Key 异常事件失败'))
  } finally {
    apiKeyRiskActionID.value = null
  }
}

async function unblockAPIKeyRiskEvent(event: APIKeyRiskEvent) {
  if (apiKeyRiskActionID.value !== null) return
  apiKeyRiskActionID.value = event.id
  try {
...
function apiKeyRiskStatusClass(status: string): string {
...
function apiKeyRiskEvidenceSummary(event: APIKeyRiskEvent): string {
...
function apiKeyRiskEvidenceSamples(event: APIKeyRiskEvent): string {
  const ev = event.evidence || {}
