...
const apiKeyAuthSnapshotVersion = 13 // v13: include user risk-control whitelist flag
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		RiskBlockedReason: apiKey.RiskBlockedReason,
		RiskBlockedAt:     apiKey.RiskBlockedAt,
		...
		User: APIKeyAuthUserSnapshot{
			...
			RiskControlWhitelisted:     apiKey.User.RiskControlWhitelisted,
		},
	}
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		RiskBlockedReason: snapshot.RiskBlockedReason,
		RiskBlockedAt:     snapshot.RiskBlockedAt,
		...
		User: &User{
			...
			RiskControlWhitelisted:     snapshot.User.RiskControlWhitelisted,
