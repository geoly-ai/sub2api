...
type User struct {
	...
	// Whether the user is exempt from API key risk-control auto-block rules
	RiskControlWhitelisted bool `json:"risk_control_whitelisted,omitempty"`
...
type UserEdges struct {
	...
	// APIKeyRiskEvents holds the value of the api_key_risk_events edge.
	APIKeyRiskEvents []*APIKeyRiskEvent `json:"api_key_risk_events,omitempty"`
...
// was not loaded in eager-loading.
func (e UserEdges) MessagesOrErr() ([]*UserMessage, error) {
	if e.loadedTypes[5] {
...
// APIKeyRiskEventsOrErr returns the APIKeyRiskEvents value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) APIKeyRiskEventsOrErr() ([]*APIKeyRiskEvent, error) {
	if e.loadedTypes[6] {
		return e.APIKeyRiskEvents, nil
	}
	return nil, &NotLoadedError{edge: "api_key_risk_events"}
}
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldTotpEnabled, user.FieldRestrictPublicGroups, user.FieldBalanceNotifyEnabled, user.FieldRiskControlWhitelisted:
...
func (_m *User) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case user.FieldRiskControlWhitelisted:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field risk_control_whitelisted", values[i])
			} else if value.Valid {
				_m.RiskControlWhitelisted = value.Bool
			}
		default:
...
func (_m *User) QueryMessages() *UserMessageQuery {
	return NewUserClient(_m.config).QueryMessages(_m)
}

// QueryAPIKeyRiskEvents queries the "api_key_risk_events" edge of the User entity.
func (_m *User) QueryAPIKeyRiskEvents() *APIKeyRiskEventQuery {
	return NewUserClient(_m.config).QueryAPIKeyRiskEvents(_m)
}
...
func (_m *User) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("risk_control_whitelisted=")
	builder.WriteString(fmt.Sprintf("%v", _m.RiskControlWhitelisted))
	builder.WriteByte(')')
