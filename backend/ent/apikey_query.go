...
type APIKeyQuery struct {
	...
	withRiskEvents *APIKeyRiskEventQuery
...
// QueryRiskEvents chains the current query on the "risk_events" edge.
func (_q *APIKeyQuery) QueryRiskEvents() *APIKeyRiskEventQuery {
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withRiskEvents: _q.withRiskEvents.Clone(),
...
// WithRiskEvents tells the query-builder to eager-load the nodes that are connected to
// the "risk_events" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithRiskEvents(opts ...func(*APIKeyRiskEventQuery)) *APIKeyQuery {
	query := (&APIKeyRiskEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRiskEvents = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withRiskEvents; query != nil {
		if err := _q.loadRiskEvents(ctx, query, nodes,
			func(n *APIKey) { n.Edges.RiskEvents = []*APIKeyRiskEvent{} },
			func(n *APIKey, e *APIKeyRiskEvent) { n.Edges.RiskEvents = append(n.Edges.RiskEvents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *APIKeyQuery) loadRiskEvents(ctx context.Context, query *APIKeyRiskEventQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *APIKeyRiskEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*APIKey)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikeyriskevent.FieldAPIKeyID)
	}
	query.Where(predicate.APIKeyRiskEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.RiskEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
