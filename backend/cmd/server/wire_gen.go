...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userMessageRepository := repository.NewUserMessageRepository(client)
	userMessageService := service.NewUserMessageService(userMessageRepository)
	userMessageHandler := handler.NewUserMessageHandler(userMessageService)
	...
	apiKeyRiskRepository := repository.NewAPIKeyRiskRepository(db)
	apiKeyRiskAPIKeyRepository := repository.NewAPIKeyRiskAPIKeyRepository(client, db)
	apiKeyRiskService := service.ProvideAPIKeyRiskService(apiKeyRiskRepository, apiKeyRiskAPIKeyRepository, userMessageService, apiKeyAuthCacheInvalidator)
	apiKeyRiskHandler := admin.NewAPIKeyRiskHandler(apiKeyRiskService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, apiKeyRiskHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, userMessageHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
